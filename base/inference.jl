...
function abstract_apply(af, aargtypes::Vector{Any}, vtypes, sv, e, isVA=false)
    if !isVA && all(x->(x<:Tuple), aargtypes) &&
        !any(x->(x.va), aargtypes[1:(length(aargtypes)-1)])
        e.head = :call1
        ...
        return abstract_call(af, (), at, vtypes, sv, (), length(aargtypes)>0 && aargtypes[end].va)
    end
    ...
    return abstract_call(af, (), Any[Any], vtypes, sv, (), true)
end

totuplety(types, va) = va ? Tuple{types..., ...} : Tuple{types...}

function abstract_call(f, fargs, argtypes::Vector{Any}, vtypes, sv::StaticVarInfo, e, isVA=false)
    if is(f,_apply) && length(fargs)>1
        af = isconstantfunc(fargs[2], sv)
        if !is(af,false)
            af = _ieval(af)
            if isa(af,Function)
                return abstract_apply(af, argtypes[3:end], vtypes, sv, e, isVA)
            end
        end
        ...
        if a2type !== Function && isleaftype(a2type)
            ...
            if isa(call_func,Function)
                ...
                return abstract_apply(call_func, aargtypes, vtypes, sv, e, isVA)
            end
        end
        return Any
    end
    if isgeneric(f)
        return abstract_call_gf(f, fargs, totuplety(argtypes, isVA), e)
    end
    if is(f,invoke) && length(fargs)>1
        ...
        if !is(af,false) && (af=_ieval(af);isgeneric(af))
            ...
            if isType(sig) && sig.parameters[1] <: Tuple
                return invoke_tfunc(af, sig.parameters[1], totuplety(argtypes[3:end], isVA))
            end
        end
    end
        ...
        if !(ff===false)
            ...
            if isgeneric(ff) && isdefined(ff.env,:kwsorter)
                ...
                return abstract_call_gf(ff.env.kwsorter, (),
                                        totuplety(Any[Array{Any,1}, posargt...], isVA), e)
            end
        end
        # TODO: call() case
        return Any
    ...
    rt = builtin_tfunction(f, fargs, totuplety(argtypes,isVA))
    #print("=> ", rt, "\n")
    return rt
end
...
function tmerge(typea::ANY, typeb::ANY)
    ...
    if (typea <: Tuple) && (typeb <: Tuple)
        if length(typea.parameters) == length(typeb.parameters) && !typea.va && !typeb.va
...
# t[n:end]
function tupletype_tail(t::ANY, n)
    p = t.parameters[n:end]
    t.va ? Tuple{p..., ...} : Tuple{p...}
end
...
function typeinf_uncached(linfo::LambdaStaticData, atypes::ANY, sparams::SimpleVector, def, curtype, cop, optimize)
    ...
    if la > 0
        ...
        if is_rest_arg(lastarg)
            ...
            if atypes === Tuple
                atypes = Tuple{Any[Any for i=1:la+1]..., ...}
            end
        end
    end
...
function inlineable(f::ANY, e::Expr, atype::ANY, sv::StaticVarInfo, enclosing_ast::Expr)
    ...
    #if is(f,tuple) && (e.typ<:Tuple) && all(isType,e.typ) && isleaftype(e.typ) && effect_free(e,sv,true)
