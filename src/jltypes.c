...
static size_t tuple_intersect_size(jl_svec_t *a, int ava, jl_svec_t *b, int bva, int *bot)
{
                                     ...
                                     cenv_t *penv, cenv_t *eqc, variance_t var)
{
    ...
    if (jl_is_tuple_type(a)) {
        ...
        if (jl_is_ntuple_type(b)) {
            ...
            if (i >= eqc->n) {
                ...
                if (((jl_datatype_t*)a)->va) {
                    b = (jl_value_t*)jl_apply_tuple_type(((jl_datatype_t*)b)->parameters, 1);
    ...
    */
    ...
    p = (jl_value_t*)jl_apply_tuple_type(super->parameters, 0);
    temp3 = (jl_value_t*)jl_apply_tuple_type(subs_sup_params, 0);
...
DLLEXPORT jl_value_t *jl_tupletype_fill(size_t n, jl_value_t *v)
{
    ...
    p = (jl_value_t*)jl_apply_tuple_type((jl_svec_t*)p, 0);
    JL_GC_POP();
    return p;
}
                                 ...
                                 jl_value_t **env, size_t n)
{
    ...
    ndt = jl_new_uninitialized_datatype(istuple ? ntp : jl_svec_len(dt->types));
...
static int jl_tuple_subtype_(jl_value_t **child, size_t cl, int cva,
                             jl_datatype_t *pdt, int ta, int invariant)
{
    ...
    size_t ci=0, pi=0;
    while (1) {
        int cseq = !ta && (ci==cl-1) && cva;
        int pseq = (pi==pl-1) && pva;
        if (cseq && !pseq)
            return 0;
        if (ci >= cl)
...
int jl_tuple_subtype(jl_value_t **child, size_t cl, int cva,
                     jl_datatype_t *pdt, int ta)
{
    return jl_tuple_subtype_(child, cl, cva, pdt, ta, 0);
}
...
static int jl_subtype_le(jl_value_t *a, jl_value_t *b, int ta, int invariant)
{
    ...
    if (jl_is_tuple_type(a)) {
        ...
        if (jl_is_tuple_type(b)) {
            return jl_tuple_subtype_(jl_svec_data(((jl_datatype_t*)a)->parameters), jl_nparams(a),
                                     ((jl_datatype_t*)a)->va, (jl_datatype_t*)b,
                                     0, invariant);
        }
    }
    ...
    if (jl_is_tuple_type(b)) {
        if (jl_is_datatype(a) &&
            ...
            if (jl_nparams(b) == 1 && ((jl_datatype_t*)b)->va) {
                return jl_subtype_le(ntp, jl_tparam0(b), 0, invariant);
            }
        }
        return 0;
    }
                               ...
                               cenv_t *env, int morespecific, int invariant)
{
    ...
    int mode = 0;
    while(1) {
        int cseq = (ci==cl-1) && child->va;
        int pseq = (pi==pl-1) && parent->va;
                               ...
                               cenv_t *env, int morespecific, int invariant)
{
    ...
    if (jl_is_tuple_type(parent)) {
        if (jl_is_datatype(child) &&
            ...
            if (jl_nparams(parent) == 1 && ((jl_datatype_t*)parent)->va) {
                return type_match_(ntp, jl_tparam0(parent), env, morespecific, invariant);
            }
        }
        return jl_false;
    }
...
void jl_init_types(void)
{
    ...
    jl_anytuple_type->parameters = jl_svec(1, jl_any_type);
    jl_anytuple_type->types = jl_anytuple_type->parameters;
    jl_anytuple_type->va = 1;
    ...
    jl_svec_t *tv = jl_svec2(tvar("N"), tvar("T"));
